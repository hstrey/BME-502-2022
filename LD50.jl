...
# ╔═╡ 4450840e-9b1d-11ec-0579-e39456a7ec07
begin
...
# ╔═╡ b3fcfc26-38e8-4d60-a4cd-eb75fdb8c6f2
function invlogit(x)
	return exp(x)/(1+exp(x))
end

# ╔═╡ a41b6c37-dc62-48ff-b2eb-5a904418f58c
begin
...
# ╔═╡ 64c1f409-9fc7-41b0-a77d-bc99eb92c8f9
...
# ╔═╡ e3883b01-7d14-4a39-95f7-27bd781e646f
poster(α,β) = posterior(α,β,dose,nd,n)

# ╔═╡ a71316c0-e208-499a-b6e9-690e42f248dc
contourf(-4:6,-5:50,poster)

# ╔═╡ 1c44d6bb-089e-4b63-9ab3-91aa5ba91fed
posterior_beta(β) = poster(2.0,β)

# ╔═╡ 0fcfc254-c32f-456a-8144-85f690d358db
plot(-5:0.1:50,posterior_beta.(-5:0.1:50))

# ╔═╡ aec1854d-3980-45eb-a28d-facde8e52373
begin
	α_range = -4.0:0.1:6.0
	β_range = -5.0:0.1:50.0
end

# ╔═╡ f5a67fbb-e456-4007-a1e6-176dcc3bcf9d
α_grid = α_range' .* ones(length(β_range))

# ╔═╡ b2c8ebe6-d634-43cc-870e-7dc5b92ec5d6
β_grid = ones(length(α_range))' .* β_range

# ╔═╡ b93868a8-6429-4bd5-94b8-389760d83f5d
p = poster.(α_grid,β_grid)

# ╔═╡ 867d0058-a0a2-485c-a42b-d65689ee1cb8
plot(β_range,vec(mean(p,dims=2)))

# ╔═╡ 42b07867-4d1b-407a-9a92-b5fbfe508734
plot(α_range,vec(mean(p,dims=1)))

# ╔═╡ 5b5a06fa-3c30-4c76-b61c-d5a555ce62b2
begin
	β_marg = vec(mean(p,dims=2))
	α_marg = vec(mean(p,dims=1))
end

# ╔═╡ 502b4a0e-00f1-45dc-b35e-3f7b4cebfb1d
function α_avg(β)
	p = poster.(α_range,β)
	return mean(p)
end
...
git-tree-sha1 = "1905aeafced631f1a703a94801003f5044fc67e3"
uuid = "28f2ccd6-bb30-5033-b560-165f7b14dc2f"
version = "0.13.0"

[[ApproxFunBase]]
deps = ["AbstractFFTs", "BandedMatrices", "BlockArrays", "BlockBandedMatrices", "Calculus", "DSP", "DomainSets", "DualNumbers", "FFTW", "FastGaussQuadrature", "FillArrays", "InfiniteArrays", "InfiniteLinearAlgebra", "IntervalSets", "LazyArrays", "LinearAlgebra", "LowRankApprox", "SparseArrays", "SpecialFunctions", "StaticArrays", "Statistics", "Test", "ToeplitzMatrices"]
git-tree-sha1 = "8102d21bbeebbd927dd9c0e30af2301e77d5e4e9"
...
git-tree-sha1 = "597451ea943d0dfd9bfc9405e1c62ec63d7de76f"
...
git-tree-sha1 = "c332d4485e3dc1252c8d159c806a3e4d9012a46c"
...
git-tree-sha1 = "848570d9a053b0e270b79a9cea95b409b317bb07"
...
git-tree-sha1 = "c3bbbd28f332a2cc8805965024b30bc4232f685c"
...
git-tree-sha1 = "e3853242083195f54b8e623949af8b8dd8f413e5"
